...
enum UITestFixtureKind: Sendable {
    case main
    case draftScroll
    case review
}
...
@MainActor
struct FixtureCommandHandler {
    let modelContext: ModelContext

    func seed(_ kind: UITestFixtureKind, now: Date) throws {
        switch kind {
        case .main:
            try seedMainFixture(now: now)
        case .draftScroll:
    ...
    private func seedMainFixture(now: Date) throws {
