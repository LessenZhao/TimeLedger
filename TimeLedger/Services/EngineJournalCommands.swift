...
struct JournalCommandHandler {
    ...
    func link(_ journal: JournalEntry, to entry: TimeEntry) throws {
        let links = try queries.digestAllLinks()
    ...
    func linkUnlinkedJournals(to entry: TimeEntry) throws -> Int {
        let journals = try queries.digestAllJournals()
        let links = try queries.digestAllLinks()
        let documents = try queries.digestAllDocuments()
    ...
    func reconcileAutoLinks() throws -> Int {
        let journals = try queries.digestAllJournals()
        let entries = try queries.digestAllTimeEntries()
        let links = try queries.digestAllLinks()
        let documents = try queries.digestAllDocuments()
        let attachments = try queries.digestAllAttachments()
    ...
    func delete(_ journal: JournalEntry) throws {
        let documents = try queries.documents(ownerID: journal.id, ownerKind: .journalEntry)
        let documentIDs = Set(documents.map(\.id))
        let attachments = try queries.attachments(documentIDs: documentIDs)
        let links = try queries.digestAllLinks()
            .filter { $0.journalEntryID == journal.id }
